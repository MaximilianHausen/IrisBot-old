...
public class PermissionAutocompleteProvider : IAutocompleteProvider
{
    public Task<IEnumerable<DiscordAutoCompleteChoice>> Provider(AutocompleteContext ctx)
    {
        if (!ctx.Interaction.GuildId.HasValue)
        {
            ...
            return !globalPermissions.Any()
                ? Task.FromResult(Array.Empty<DiscordAutoCompleteChoice>() as IEnumerable<DiscordAutoCompleteChoice>)
                : Task.FromResult(new List<DiscordAutoCompleteChoice>(globalPermissions.Select(p => new DiscordAutoCompleteChoice(p, p))) as IEnumerable<DiscordAutoCompleteChoice>);
        }
        ...
        return !permissions.Any()
            ? Task.FromResult(Array.Empty<DiscordAutoCompleteChoice>() as IEnumerable<DiscordAutoCompleteChoice>)
            : Task.FromResult(new List<DiscordAutoCompleteChoice>(permissions.Select(p => new DiscordAutoCompleteChoice(p, p))) as IEnumerable<DiscordAutoCompleteChoice>);
    }
}
